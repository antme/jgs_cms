...
function getMisUser(){
	...
	if (isset ( $user -> session )) {
		if (!strpos ( $user->session, "USER_TYPE_NAME施工单位" ) && strpos ( $user->session, "USER_TYPE_NAME施工单位" ) > 0) {				
			return "施工单位";
		}
		if (!strpos ( $user->session, "USER_TYPE_NAME施工单位" ) && strpos ( $user->session, "USER_TYPE_NAME监理单位" ) > 0) {
			
			return "监理单位";
		}
		if (!strpos ( $user->session, "USER_TYPE_NAME施工单位" ) && strpos ( $user->session, "USER_TYPE_NAME建设单位" ) > 0) {
